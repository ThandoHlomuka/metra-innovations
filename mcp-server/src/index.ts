...
//
// Exposes tools + resources so AI agents (Claude, ChatGPT, etc.) can:
//   - answer questions about the company, services, portfolio and contact details
//   - submit a contact query on a visitor's behalf
//   - generate a WhatsApp deep link so a user can continue the conversation in
//     WhatsApp with the company (the message is pre-filled by the agent).
//
// Transports (Node.js / stdio is the primary, zero-config path used by local
// integrations like Claude Desktop and Claude Code):
//   - stdio (default)
//   - streamable HTTP (--http, requires the optional @modelcontextprotocol/node
//     middleware for remote/Connector use)
...
server.registerTool(
  ...
  {
    description:
      'Get the full list of services offered by Metra Innovations (web development, ecommerce, ' +
...
server.registerTool(
  ...
  {
    description:
      'Generate a WhatsApp deep link to contact Metra Innovations from a pre-filled message. ' +
...
// ---------------------------------------------------------------
// Resources (read-only company data an agent can browse)
// ---------------------------------------------------------------
...
server.registerResource(
  'company',
  'metra://company',
  { title: 'Metra Innovations - Company Overview', description: 'Company profile: name, location, contact, hours and description.', mimeType: 'application/json' },
  companyResource
);
...
server.registerResource(
  'services',
  'metra://services',
  { title: 'Metra Innovations - Services', description: 'All services offered by Metra Innovations.', mimeType: 'application/json' },
  () => ({ contents: [{ uri: 'metra://services', text: JSON.stringify(SERVICES, null, 2) }] })
);

server.registerResource(
  'portfolio',
  'metra://portfolio',
  { title: 'Metra Innovations - Portfolio', description: 'Case studies / projects built by Metra Innovations.', mimeType: 'application/json' },
  () => ({
    contents: [
      {
        uri: 'metra://portfolio',
...
server.registerResource(
  'contact',
  'metra://contact',
  { title: 'Metra Innovations - Contact & WhatsApp', description: 'Contact details and WhatsApp deep link for Metra Innovations.', mimeType: 'application/json' },
  () => ({
    contents: [
      {
        uri: 'metra://contact',
...
server.registerResource(
  'reference',
  'metra://reference',
  { title: 'Metra Innovations - Reference Data', description: 'Valid service ids, budget ranges and SEO keywords.', mimeType: 'application/json' },
  () => ({
    contents: [
      {
        uri: 'metra://reference',
...
server.registerResource(
  'queries',
  'metra://queries',
  { title: 'Metra Innovations - Recorded Queries (read-only)', description: 'List of contact queries recorded via submit_contact_query.', mimeType: 'application/json' },
  async () => ({
...
if (!useHttp) {
  ...
  try {
    const { NodeStreamableHTTPServerTransport, localhostHostValidation, localhostOriginValidation } =
    ...
    const httpServer = createServer((req, res) => {
      // DNS-rebinding + cross-origin protection (local-only defaults).
    ...
    console.error(
      'streamable-http transport requires the @modelcontextprotocol/node middleware.\n' +
